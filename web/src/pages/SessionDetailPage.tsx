...
function TranscriptViewer({ recordingId }: { recordingId: string }) {
  ...
  return (
    <div className="max-h-72 overflow-y-auto rounded-2xl bg-panel-2 p-3 text-sm ring-1 ring-edge">
      {segments.map((seg, i) => (
...
export function SessionDetailPage() {
  ...
  if (!session) return <div className="min-h-screen bg-ink p-6 text-sm text-gray-400">Loading…</div>;
      ...
      <main className="mx-auto max-w-5xl p-6">
        ...
        {participants.length > 0 && (
          <Card className="mt-6 !rounded-2xl !border-0 !bg-panel ring-1 ring-edge">
            <h2 className="text-xs font-semibold uppercase tracking-wider text-gray-500">Participants</h2>
        ...
        {recordings.map((recording, index) => {
          ...
          return (
            <Card key={recording.id} className="mt-6 !rounded-2xl !border-0 !bg-panel ring-1 ring-edge">
            ...
            {/* Tracks */}
            <div className="mt-4 overflow-x-auto rounded-2xl bg-panel-2 ring-1 ring-edge">
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-edge text-left text-xs uppercase tracking-wider text-gray-500">
                    <th className="px-3 py-2 pr-4">Participant</th>
                ...
                <tbody>
                  {recTracks.map((track) => (
                    <tr key={track.id} className="border-b border-edge/50">
                      <td className="px-3 py-2 pr-4">{track.participant_name}</td>
                      ...
                      <td className="py-2 pr-3">
                        {track.status === "ready" ? (
                          <div className="flex gap-2 text-xs">
                            {track.width !== null && (
                              <a className="text-accent-2 hover:underline" href={`/api/tracks/${track.id}/download?kind=mp4`}>MP4</a>
                            )}
                            <a className="text-accent-2 hover:underline" href={`/api/tracks/${track.id}/download?kind=wav`}>WAV</a>
                            <a className="text-accent-2 hover:underline" href={`/api/tracks/${track.id}/download?kind=raw`}>Raw</a>
                            {track.enhanced ? (
                          ...
                          <button onClick={() => void reprocess(track.id)} className="text-xs text-accent-2 hover:underline">
                            Retry processing
                          </button>
                        ) : (
            ...
            {transcript?.status === "ready" && (
              <div className="mt-3">
                <div className="flex items-center gap-3 text-xs">
                  ...
                  <a className="text-accent-2 hover:underline" href={`/api/recordings/${recording.id}/transcript/download?format=txt`}>TXT</a>
                  <a className="text-accent-2 hover:underline" href={`/api/recordings/${recording.id}/transcript/download?format=srt`}>SRT</a>
                  <a className="text-accent-2 hover:underline" href={`/api/recordings/${recording.id}/transcript/download?format=vtt`}>VTT</a>
            ...
            {recExports.length > 0 && (
              <div className="mt-3 flex flex-col gap-2">
                {recExports.map((exp) => (
                  <div key={exp.id} className="flex items-center justify-between rounded-2xl bg-panel-2 px-3 py-2 text-sm">
                    ...
                    <span className="flex items-center gap-3">
                      ...
                      {exp.status === "ready" && (
                        <a className="text-xs text-accent-2 hover:underline" href={`/api/exports/${exp.id}/download`}>
                          Download
                        </a>
                      )}
                      {exp.status === "ready" && exp.has_captions && (
                        <a className="text-xs text-accent-2 hover:underline" href={`/api/exports/${exp.id}/captions`}>
                          SRT
                        </a>
                      )}
                      {exp.error && (
